...
Aggregate the total monetary damage by weather type
...
head(dfEconWorst[with(dfEcon, order(-prop.damage)),], 10L);

# order ascending by crop damage
head(dfEconWorst[with(dfEcon, order(-crop.damage)),], 10L);

# order ascending by net damage
head(dfEconWorst[with(dfEcon, order(-net.damage)),], 10L);
```
