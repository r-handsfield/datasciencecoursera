...
```{r}
vecTrn <- createDataPartition(y = dfTrn$classe, p = 0.1, list = FALSE)
...
```{r}
trn <- trn[complete.cases(trn), 8:60]
val <- val[complete.cases(val), 8:60]
...
# mod <- train(x = trn, y = trn$classe, method = "rf")
m <- train(form = classe~., data = trn, method = "rf")
```

```{r cache=TRUE, message=FALSE}
p <- predict(object = mod, newdata = val)
```

```{r}
confusionMatrix(data = p, reference = val$classe)
```
