...
```{r echo=FALSE}
k <- "The sample means are normally distributed and centered near the population mean (red line)."
```

```{r warning=FALSE, message=FALSE, fig.align='center', fig.cap=k}
...
<br />
The variance of this distribution of sample means $Var(\bar X_\mu)$ is
```{r}
var(means);
```

The probability of the sample means randomly centering at `r round(M,2)` is:
```{r}
...
## Overview
The `ToothGrowth` data set reports _The Effect of Vitamin C on Tooth Growth in Guinea Pigs_ in 3 columns: tooth length, supplement type, and dose.
...
The data set reports the tooth length in each of 10 guinea pigs receiving one of three doses of Vitamin C (0.5, 1, 2 mg), with one of two delivery methods (orange juice or ascorbic acid). Accounting for dosage and delivery method, there are 6 independent groups of 10 guinea pigs.

| __Column__  | `len`        | `supp` 	 		| `dose`    |
|-------------|--------------|--------------------------|-----------|
| __Type__    | `numeric`    | `factor`                 | `numeric` |
| __Content__ | tooth length | supplement type {VC, OJ} | dose (mg) |
...
g <- g + facet_grid(.~supp) + xlab("Dose (mg)") + ggtitle("Tooth Growth by Supplement & Dose")
g
```
